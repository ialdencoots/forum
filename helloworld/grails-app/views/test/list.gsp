			...
			<table>
				<thead>
					<tr>
						...
						<g:sortableColumn property="password" title="${message(code: 'test.password.label', default: 'Password')}" />
					
						<g:sortableColumn property="userName" title="${message(code: 'test.userName.label', default: 'User Name')}" />
				...
				<g:each in="${testInstanceList}" status="i" var="testInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${testInstance.id}">${fieldValue(bean: testInstance, field: "password")}</g:link></td>
					
						<td>${fieldValue(bean: testInstance, field: "userName")}</td>
